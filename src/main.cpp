...
#if defined(NDEBUG)
# error "SelfDirectedCoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x720c1e2489cea263249181908d8bc4ee8ab85886a60541981d4290e9eafd203a");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // SelfDirectedCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "SelfDirectedCoin Signed Message:\n";
...
{
    // SelfDirectedCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // SelfDirectedCoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // SelfDirectedCoin: 1 day (difficulty readjustment)
static const int64 nTargetSpacing = 1 * 60; // SelfDirectedCoin: 1 minute (target time to find block)
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // SelfDirectedCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // SelfDirectedCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xc7fbed3a3cf4a6cfbaaca87db86c0cd983fc37c11d1c822e03360bbf89108ecd");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Guardian 10/Jan/2018 World Bank issues warnings on interest rates and inflation";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04b5a1c4d739cdee02b815790e60b63c722e8036ee7dd95b3df8841a0c7ca6285eeffe2309cf6dc6c441edb6673f653b1be780ef94b049aade05041adc3b17d9e3") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1515575154;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084545674;

        if (fTestNet)
        {
            block.nTime    = 1515575128;
            block.nNonce   = 386035719;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xe7e9faf6f6892d98b82a7446fbe38cba30034be283102ae61c45e1881b8dbf17"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfe, 0xc3, 0xb8, 0xda }; // SelfDirectedCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SelfDirectedCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SelfDirectedCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SelfDirectedCoinMiner(CWallet *pwallet)
{
    printf("SelfDirectedCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("SelfDirectedCoin-miner");
    ...
    try { loop {
        ...
        printf("Running SelfDirectedCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SelfDirectedCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SelfDirectedCoinMiner, pwallet));
}
